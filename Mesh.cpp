...
void Mesh::FillBGVertex() {
	glGenVertexArrays(1, &m_vertexArrayObject[1]);
	...
	glBufferData(GL_ARRAY_BUFFER, 6 * sizeof(bgVecs[0]), &bgVecs[0], GL_STATIC_DRAW);
	glEnableVertexAttribArray(2);//1:着色器中属性位置索引
	glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 0, 0);
}
void Mesh::FillVertex(vector<Vertex> vertices, unsigned int numVertices, unsigned int* indices, unsigned int numIndices)
{
	...
	*/
	...
	glBufferData(GL_ARRAY_BUFFER, numVertices * (sizeof(positios[0]) + sizeof(texCoord[0])), NULL, GL_DYNAMIC_DRAW);
	glBufferSubData(GL_ARRAY_BUFFER, 0, numVertices * sizeof(positios[0]), &positios[0]);
	glBufferSubData(GL_ARRAY_BUFFER, numVertices * sizeof(positios[0]), numVertices * sizeof(texCoord[0]), &texCoord[0]);
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, (const GLvoid*)(numVertices * sizeof(positios[0])));
	/*关联一个顶点属性数组
		参数1:对应着色器中属性位置
	...
	*/
	...
	////int offset = numVertices * sizeof(positios[0]) * sizeof(float);
	//glEnableVertexAttribArray(1);//1:着色器中属性位置索引
	//glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, 0);
	/*关联一个顶点属性数组
		参数1:对应着色器中属性位置
